...
from catalog.models import Category, Items, User
...
def editCategories(category_id):
    categories = session.query(Category).order_by(asc(Category.name))
    editCategory = session.query(Category).filter_by(id=category_id).first()
...
def editItem(category_id, item_id):
    ...
    editItem = session.query(Items).filter_by(id=item_id).first()
